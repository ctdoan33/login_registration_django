...
class Manager(models.Manager):
    def register_validator(self, postData):
        ...
        try:
            if datetime.strptime(postData["birthdate"], "%Y-%m-%d") > datetime.today():
                errors["birthdate"] = "Birthdate cannot be in the future"
            if datetime.strptime(postData["birthdate"], "%Y-%m-%d") < datetime.today() - timedelta(days=46000):
                errors["birthdate"] = "No living persons over 125 years old"
        except ValueError:
